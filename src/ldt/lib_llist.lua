...
-- (*) List   = llist.scan( topRec, ldtBinName )
-- (*) List   = llist.filter( topRec, ldtBinName, userModule, filter, fargs )
...
-- ======================================================================
local function leafInsert(src, topRec, leafRec, ldtMap, newKey, newValue, position)
...
local function createNodeRec( src, topRec, ldtCtrl )
  ...
  -- NodePropMap is already set up from the createSubRec() call.
  local nodeCtrlMap = map();
...
-- ======================================================================
local function createLeafRec( src, topRec, ldtCtrl, firstValue )
  ...
  -- Set up our new property and control map for this leaf
  -- leafPropMap is already set up from the createSubRec() call.
  local leafCtrlMap = map();

  -- Store the new maps in the record.
  -- leafSubRec[SUBREC_PROP_BIN] = leafPropMap; (Already Set)
  leafSubRec[NSR_CTRL_BIN]    = leafCtrlMap;
  leafSubRec[NSR_KEY_LIST_BIN] = list(); -- Holds the keys
  leafSubRec[NSR_DIGEST_BIN] = list(); -- Holds the Digests -- the Rec Ptrs
  ...
  if( ldtMap[R_StoreMode] == SM_LIST ) then
    ...
    if( firstValue ~= nil ) then
      ...
      leafCtrlMap[LF_ListEntryCount] = 1;
      leafCtrlMap[LF_ListEntryTotal] = 1;
    else
  ...
  -- ldt_common.addSubRecToContext( src, leafRec, true );
...
local function firstTreeInsert( src, topRec, ldtCtrl, newValue, stats )
  ...
  local rightLeafRec = createLeafRec( src, topRec, ldtCtrl, newValue);
...
local function treeInsert( src, topRec, ldtCtrl, value, stats )
  ...
  if( ldtMap[R_TreeLevel] == 1 ) then
    ...
    if( sp.HasRoom[leafLevel] == true ) then
      ...
      rc = leafInsert(src, topRec, leafRec, ldtMap, key, value, position);
      -- We no longer call udpate -- it is done at the close of Lua Context.
      -- aerospike:update_subrec( leafRec );
    else
      ...
      rc = splitLeafInsert( src, topRec, sp, ldtCtrl,
                            key, value );
    end
...
local function getNextLeaf( src, topRec, leafRec  )
  ...
  ldt_common.closeSubRec( src, leafRec );
...
-- (*) List   = llist.first( topRec, ldtBinName, count )
-- (*) List   = llist.last( topRec, ldtBinName, count )
-- (*) List   = llist.range( topRec, ldtBinName, lowValue, highValue ) 
--
-- ======================================================================
...
-- (*) createSpec:
-- =======================================================================
function llist.add( topRec, ldtBinName, newValue, createSpec )
  GP=B and trace("\n\n >>>>>>>>> API[ LLIST ADD ] <<<<<<<<<<< \n");
...
-- =======================================================================
function llist.scan( topRec, ldtBinName )
  GP=B and trace("\n\n  >>>>>>>>>>>> API[ SCAN ] <<<<<<<<<<<<<< \n");
  local meth = "scan()";
...
function llist.filter( topRec, ldtBinName, userModule, filter, fargs )
  GP=F and trace("\n\n  >>>>>>>>>>>> API[ FILTER ]<<<<<<<<<<< \n");
  ...
  return llist.find( topRec, ldtBinName,nil, userModule, filter, fargs );
end -- llist.filter()
...
-- ======================================================================
function llist.remove( topRec, ldtBinName, key )
  GP=F and trace("\n\n  >>>>>>>>>>>> API[ REMOVE ]<<<<<<<<<<< \n");
...
-- ========================================================================
function llist.destroy( topRec, ldtBinName )
  GP=B and trace("\n\n >>>>>>>>> API[ LLIST DESTROY ] <<<<<<<<<< \n");
...
-- ========================================================================
-- llist.debug() -- Turn the debug setting on (1) or off (0)
-- ========================================================================
...
--   res = -1: Some sort of error
-- ========================================================================
function llist.debug( topRec, setting )
  local meth = "llist.debug()";
  local rc = 0;
  ...
  if( setting ~= nil and type(setting) == "number" ) then
    ...
    elseif( setting == 0 ) then
      ...
      trace("[DEBUG SET]<%s:%s> BAD Setting(%s)",MOD,meth,tostring(setting));
      rc = -1;
    end
  else
    trace("[DEBUG SET]<%s:%s> BAD Setting(%s)",MOD,meth,tostring(setting));
